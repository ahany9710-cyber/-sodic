...
import { ogamiPaymentIcon } from './ogamiIconMap';
...
const OgamiPaymentPlan = () => {
  ...
  const plans = p.plans.map((plan, idx) => ({
    ...plan,
    iconKey: p.icons[idx] as keyof typeof ogamiPaymentIcon,
  }));

  return (
    <section id="ogami-payment" className="relative overflow-hidden bg-black px-6 py-16 md:px-16 md:py-24">
      <div className="mx-auto max-w-[1600px]">
        ...
        <div className="grid grid-cols-1 gap-5 md:grid-cols-3">
          {plans.map((plan, index) => {
            const Icon = ogamiPaymentIcon[plan.iconKey];
            return (
              <motion.div
                key={plan.id}
